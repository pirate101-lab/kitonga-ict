...
export function FastOrderForm() {
  ...
  const [turnaround, setTurnaround] = useState(TURNAROUNDS[0] ?? "Standard");
  const [budget, setBudget] = useState(BUDGETS[1] ?? "Flexible");
  ...
  return (
    <div className="grid gap-6 lg:grid-cols-[1fr_360px] lg:items-start">
      {/* Form */}
      <div className="rounded-2xl border border-[#cbd5e1]/70 bg-card p-5 md:p-7">
                ...
                <label
                  ...
                  className={`flex cursor-pointer items-start gap-2.5 rounded-xl border p-2.5 transition-colors ${
                    serviceId === s.id
                      ...
                      : "border-[#cbd5e1]/70 bg-card text-foreground hover:border-primary hover:bg-secondary"
                  }`}
                >
                  <input
                    type="radio"
                  ...
                  <span
                    className={`mt-0.5 grid h-8 w-8 place-items-center rounded-lg shrink-0 ${
                      serviceId === s.id
                        ...
                        : "border border-[#cbd5e1]/70 bg-secondary text-primary"
                    }`}
                    aria-hidden
                  >
                    <s.icon size={14} />
      ...
      <div className="lg:sticky lg:top-24 flex flex-col gap-3">
        ...
        <div className="rounded-2xl border border-[#cbd5e1]/70 bg-card p-4">
          <div className="flex items-center gap-2 border-b border-[#cbd5e1]/70 pb-3">
        ...
        <div className="rounded-2xl border border-[#cbd5e1]/70 bg-card p-3.5 text-[11.5px] text-muted-foreground">
          Hitting{" "}
...
const inputClass =
  "w-full rounded-xl border border-[#cbd5e1]/70 bg-card px-3.5 py-2.5 text-sm text-foreground placeholder:text-[#888] focus:outline-none focus:border-primary focus:ring-2 focus:ring-primary/30 transition";
    ...
    <button
      ...
      className={`rounded-full border px-2.5 py-1.5 text-[11.5px] font-semibold transition-colors ${
        active
          ...
          : "border-[#cbd5e1]/70 bg-card text-muted-foreground hover:border-primary hover:text-primary"
      }`}
    >
      {children}
    </button>
