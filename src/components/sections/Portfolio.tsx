              ...
              <button
                ...
                className={`rounded-full border px-3.5 py-1.5 text-[12px] font-medium transition-colors ${
                  isActive
                    ...
                    : "border-black/30 bg-white text-[#333] hover:border-[#0067b8] hover:text-[#0067b8]"
                }`}
              >
                {f}
              </button>
        ...
        {loading ? (
          <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
            {Array.from({ length: limit ?? 6 }).map((_, i) => (
              <div
                key={i}
                className="aspect-[4/3] rounded-2xl border-2 border-black/30 bg-white animate-pulse"
              />
            ))}
          </div>
        ) : filtered.length === 0 ? (
          ...
          <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
            {filtered.map((item) => (
              <article
                key={item.id}
                className="group relative flex flex-col overflow-hidden rounded-2xl border-2 border-black/30 bg-white hover:border-[#0067b8] transition-colors"
              >
                {/* Image / mockup */}
                ...
                <div className="flex items-start justify-between gap-2 px-4 py-3">
                  ...
                  <span className="shrink-0 rounded-full border-2 border-black/30 bg-[#f2f5f9] px-2.5 py-0.5 text-[10.5px] font-medium text-[#333] uppercase tracking-wide">
                    {item.category}
                  </span>
                </div>
              </article>
